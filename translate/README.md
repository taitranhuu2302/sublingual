...
- debounce on both the Vosk side and the translate side for better stability
...
```bash
docker compose -f docker/docker-compose.yml up --build translate
```
...
- separates `translate` for the API and `model-builder` for conversion tasks
